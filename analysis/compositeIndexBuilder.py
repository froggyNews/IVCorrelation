"""
Builders for Index-style composite volatility surfaces (grid & ATM-pillar).
...
def combine_surfaces(
    ...
    Weighted grid combination into a composite surface (per date):
        sigma_composite(K,T) = sum_i rho_i * w_i(K,T) * sigma_i(K,T) / sum_i rho_i * w_i(K,T)
    """
...
def build_composite_iv_by_rank(
    ...
    Returns DataFrame with columns: ["rank", "composite_iv"].
    """
    ...
    if atm_df.empty:
        return pd.DataFrame(columns=["rank", "composite_iv"])
    ...
    for r in atm_df.columns:
        ...
        if ws:
            rows.append({"rank": int(r), "composite_iv": float(np.dot(ws, ivs)) / float(np.sum(ws))})
