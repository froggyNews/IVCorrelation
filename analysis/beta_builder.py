...
def cosine_similarity_weights(X_peers: np.ndarray, y_target: np.ndarray) -> np.ndarray:
    """
    Compute weights based on cosine similarity between target and peers.
    
    Parameters:
    - X_peers: (n_peers x n_features) array of peer data
    - y_target: (n_features,) array of target data
    
    Returns:
    - weights: (n_peers,) array of normalized weights
    """
    # Impute missing values
    X_peers_clean = _impute_col_median(X_peers)
    y_target_clean = _impute_col_median(y_target.reshape(1, -1)).ravel()
    
    # Calculate cosine similarities manually
    similarities = []
    target_norm = np.linalg.norm(y_target_clean)
    
    if target_norm == 0:
        # If target is zero vector, return equal weights
        return np.full(X_peers_clean.shape[0], 1.0 / X_peers_clean.shape[0])
    
    for peer in X_peers_clean:
        peer_norm = np.linalg.norm(peer)
        if peer_norm == 0:
            sim = 0
        else:
            # Cosine similarity = dot product / (norm1 * norm2)
            sim = np.dot(peer, y_target_clean) / (peer_norm * target_norm)
    ...
    if total_sim > 0:
        ...
        return np.full(len(similarities), 1.0 / len(similarities))
...
def pca_weights_from_atm_matrix(
    ...
    w_all = _first_pc_weights_from_rows_improved(Z, ridge=ridge)
    ser = pd.Series(w_all, index=kept_names).drop(index=[target], errors="ignore")
    ser = ser.reindex(peers).fillna(0.0)
    # Note: clip_negative is not applied since the improved method already handles negative weights
    s = ser.sum()
...
def pca_weights(
    ...
    if mode.startswith("pca_atm"):
        ...
        if "cosine" in mode:
            w = cosine_similarity_weights(Xp, y)
        else:
    ...
    if mode.startswith("pca_surface"):
        ...
        if "market" in mode:
            ...
            w = cosine_similarity_weights(X[1:, :], X[0, :])
        else:
