...
def main() -> int:
    ...
    try:
        if getattr(args, "slide10_smile", False):
            ...
            if df_tgt is None or df_tgt.empty:
                ...
                try:
                    ...
                    # Normalize to a plain dict with positive weights only
                    ...
                    if isinstance(w_map, dict) and len(w_map) > 0:
                        ...
                        peer_surfaces = {t: surfaces[t] for t in slide_peers if t in surfaces}
                        synth_by_date = combine_surfaces(peer_surfaces, w_map)
                        # Choose a common date
                        tgt_dates = set(surfaces.get(target, {}).keys())
                        syn_dates = set(synth_by_date.keys())
                        common = sorted(tgt_dates & syn_dates)
                        if common:
                            date_use = common[-1]
                            tgt_grid = surfaces[target][date_use]
                            syn_grid = synth_by_date[date_use]
        ...
        print(f"Slide10 smile figure error: {e}")
